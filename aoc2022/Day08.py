def p1(data):
    ...
    for i, line in enumerate(data[1:-1]):
        for j, t in enumerate(line[1:-1]):
            if all(t > e for e in line[:j+1]) or all(t > e for e in line[j + 2:]) or all(
                    t > e for e in [row[j+1] for row in data[:i+1]]) or all(
...
def p2(data):
    ...
    for i, line in enumerate(data[1:-1]):
        for j, t in enumerate(line[1:-1]):
            left, right, up, down = [t > e for e in line[:j+1]][::-1], [t > e for e in line[j + 2:]], [t > e for e in [row[j+1] for row in data[:i+1]]][::-1], [t > e for e in [row[j+1] for row in data[i + 2:]]]
